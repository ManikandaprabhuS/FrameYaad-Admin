...
const CustomerProductDetailsPage: React.FC = () => {
  ...
  const [croppedImages, setCroppedImages] = useState<Record<string, string>>(() => {
    if (!slug) return {};
    try {
      return JSON.parse(window.localStorage.getItem(`frameyaad-crops:${slug}`) || '{}') as Record<string, string>;
    } catch {
      return {};
    }
  });
  ...
  const frameSizes = uniqueValues(activeVariants.map((variant) => variant.frameSize));
  const mountTypes = uniqueValues(activeVariants.map((variant) => variant.mountType));
  const glassTypes = uniqueValues(activeVariants.map((variant) => variant.glassType));
  ...
  return (
    <div className="bg-[#fafafa] pb-12 text-black">
      <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8">
        <div className="mb-4 flex flex-wrap items-center gap-2 text-[11px] text-black/50">
              ...
              <div className="order-2 flex gap-2 overflow-x-auto sm:order-1 sm:flex-col sm:overflow-visible">
                {galleryImages.map((image) => (
                  <button key={image.id} type="button" onClick={() => setSelectedGalleryImage(image.imageUrl)} className={`h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-white ${mainImage === image.imageUrl ? 'border-black' : 'border-transparent hover:border-black/25'}`}>
                    <img src={image.imageUrl} alt={`${currentProduct.name} view`} className="h-full w-full object-cover" />
                  </button>
                ))}
                {croppedImage && (
                  <button type="button" onClick={() => setSelectedGalleryImage(croppedImage)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-white ${mainImage === croppedImage ? 'border-black' : 'border-transparent'}`}>
                    <img src={croppedImage} alt="Your cropped photo" className="h-full w-full object-cover" />
                    <span className="absolute bottom-0 inset-x-0 bg-black/75 py-0.5 text-[8px] font-bold text-white">YOUR PHOTO</span>
                  </button>
                )}
              </div>

              <div className="order-1 relative min-h-[360px] overflow-hidden rounded-xl border border-black/10 bg-[#eeeae4] sm:order-2 lg:min-h-[560px]">
                ...
                {mainImage ? <img src={mainImage} alt={currentProduct.name} className="h-full w-full object-cover object-center" /> : <div className="grid h-full min-h-[360px] place-items-center text-sm text-black/40">No product image</div>}
                {croppedImage && mainImage === croppedImage && (
                  <div className="pointer-events-none absolute inset-0 grid place-items-center">
                    <div className="border-[12px] border-black/75 shadow-[0_20px_50px_rgba(0,0,0,0.3)]" style={{ width: '62%', aspectRatio: `${frameRatio.width}/${frameRatio.height}` }}>
                      <img src={croppedImage} alt="Cropped preview inside selected frame" className="h-full w-full object-cover" />
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-black/10 bg-white p-4">
                ...
                <button type="button" onClick={() => croppedImage ? setCropSource(croppedImage) : fileInputRef.current?.click()} className="mt-3 flex h-28 w-full items-center justify-center overflow-hidden rounded-lg bg-[#eeeae4]">
                  {croppedImage ? <img src={croppedImage} alt="Cropped photo preview" className="h-full w-full object-cover" /> : <span className="flex flex-col items-center gap-2 text-xs font-bold"><ImagePlus className="h-6 w-6" /> Upload to preview</span>}
                </button>
              </div>
            </div>
          </div>

          <aside className="self-start rounded-xl border border-black/10 bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.05)] xl:sticky xl:top-20">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <button type="button" onClick={() => showSuccess('Your configured frame is ready. Cart checkout will use this selection when connected.')} disabled={!selectedVariant || Number(selectedVariant.stockQuantity) <= 0} className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-lg bg-black text-xs font-black text-white disabled:bg-black/30"><ShoppingBag className="h-4 w-4" /> Add to Bag</button>
            </div>
...
const ProductDetailsSkeleton = () => (
  <div className="mx-auto grid max-w-[1500px] animate-pulse gap-6 px-4 py-8 lg:grid-cols-2">
    <div className="min-h-[560px] rounded-xl bg-black/10" />
